...
ROS 2 bag, HDF5, Arrow IPC, web/cloud services, Flight Review, dataset publishing, ArduPilot, and raw MAVLink are outside v0.1 and are not assumed future core work. Existing maintained ecosystem converters should be documented before any adapter is considered.
...
5. **Scale.** Collecting hundreds of wide dataframes exhausts memory. Current resolution: bounded worker count and one completed flight returned to the coordinator at a time. DataFrame process-transfer overhead remains unbenchmarked and is documented rather than hidden.
...
- Deterministic rules/splits for identical inputs and configuration; the manifest records wall-clock generation time separately.
- No network calls or telemetry.
- A configurable normalized-row bound; parser and multiprocessing memory remain input-dependent.
